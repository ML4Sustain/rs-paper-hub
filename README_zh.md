...
- **按作者自动更新研究组** — 设置 `"auto": true` 和 `"authors"` 字段后，pipeline 自动匹配论文并每日更新
- **今日新增标记** — 统计栏显示 `+N` 新增数量
...
- **雷达页** — 3D 地球词云（基于 BERT 加权关键词，可拖拽旋转、点击关键词搜索），旁边展示本周热门论文排行，支持三种评分模式（趋势匹配 / 词频热度 / 综合评分）及数据源切换（全部 / VLM / UAV / Agent / SAR）
- **移动端适配** — 可折叠筛选面板，响应式布局
